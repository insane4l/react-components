...
export const SuperRange: React.FC<SuperRangePropsType> = (
    {
        ...
        className, withValueIndicator,
        max = 100, 
        // todo: mb add 'width' property
        // or 'style' will be better (wrapper style)
    ...
    const finalRangeClassName = `superRange__input ${className ? className : ''}`


    return (
        <div className="superRange__slider-wrapper">
            ...
            {withValueIndicator && (restProps.value || restProps.value === 0) && <ValueIndicator currentValue={+restProps.value} maxValue={+max} />}        
        </div>
    )
}
...
const SelectedRangeLine: React.FC<{currentValue: number, maxValue: number}> = ({currentValue, maxValue}) => {
    ...
    return (
        <div style={style} className="superRange__selected_line"></div>
    )
}
...
const ValueIndicator: React.FC<{currentValue: number, maxValue: number}> = ({currentValue, maxValue}) => {
    ...
    return (
        <div style={{left: valueIndicatorPosition}} className="superRange__value_indicator" >
            <div>
                {currentValue}
            </div>
        </div>
